...
class TestInitialization:
    ...
    def test_init_with_postgres(self):
        ...
        db_config = {"db_url": "postgresql+psycopg2://user:pass@localhost:5432/testdb"}
...
class TestConcurrency:
    ...
    def test_api_cache_thread_safety(self, tmp_path):
        ...
        def cache_worker():
            try:
                endpoint_id = engine._get_or_create_endpoint_id(
...
class TestFailureSimulation:
    ...
    def test_worker_executor_failure(self, tmp_path):
        ...
        with patch.object(
            engine.worker._executor, "submit", side_effect=Exception("Executor error")
        ):
            data = {
                "api_url": "/test",
                "start_dt": datetime.now(timezone.utc),
    ...
    def test_middleware_exception_handling(self, tmp_path):
        ...
        with patch.object(
            middleware.engine, "capture", side_effect=Exception("Capture error")
        ):
...
class TestBulkMode:
    ...
    def test_bulk_mode_enabled(self, tmp_path):
        ...
        db_config = {"db_url": f"sqlite:///{db_path}"}
        tracelet.init(
            db_config=db_config, enabled=True, use_bulk_mode=True, batch_size=5
        )
