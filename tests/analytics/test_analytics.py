...
def test_analytics_engine():
    ...
    try:
        ...
        if not earliest:
            console.print(
                "[yellow]    ⚠️  No data found - run generate_test_data first[/yellow]"
            )
            return
        ...
        console.print(
            f"    ✓ Endpoint: {endpoints[0]['method']} {endpoints[0]['path']}"
        )
        ...
        batch_stats = engine.fetch_batch_summary_stats(
            endpoint_ids, start_time, end_time
        )
...
def test_analytics_service():
    ...
    try:
        ...
        for window_str in windows_to_test:
            ...
            if window:
                console.print(
                    f"    ✓ '{window_str}' → {window.label} ({window.duration_human()})"
                )
...
def run_all_tests():
    ...
    console.print(
        "[bold cyan]║         Tracelet Analytics Test Suite                       ║[/bold cyan]"
    )
    console.print(
        "[bold cyan]╚═══════════════════════════════════════════════════════════════╝[/bold cyan]"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--engine", action="store_true", help="Test AnalyticsEngine only"
    )
    parser.add_argument(
        "--percentile", action="store_true", help="Test percentile estimation only"
    )
    parser.add_argument(
        "--service", action="store_true", help="Test AnalyticsService only"
    )
