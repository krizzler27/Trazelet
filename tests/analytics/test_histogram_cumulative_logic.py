...
def test_delta_to_cumulative_conversion():
    ...
    deltas = [
        HistogramSnapshot(
            threshold_ms=100.0, cumulative_count=150
        ),  # 150 new requests ≤ 100ms
        HistogramSnapshot(
            threshold_ms=250.0, cumulative_count=100
        ),  # 100 new requests ≤ 250ms
        HistogramSnapshot(
            threshold_ms=500.0, cumulative_count=50
        ),  # 50 new requests ≤ 500ms
    ]
    ...
    for snapshot in sorted_deltas:
        ...
        cumulative.append(
            HistogramSnapshot(
                threshold_ms=snapshot.threshold_ms, cumulative_count=running_total
            )
        )
...
def test_percentile_calculation_with_deltas():
    ...
    assert 10.0 <= p50 <= 25.0, f"P50 ({p50}ms) should be between 10ms and 25ms"

    # Calculate P95 - should be higher
    p95 = estimate_percentile(tuple(deltas), 95.0)
    assert p95 > p50, f"P95 ({p95}ms) should be greater than P50 ({p50}ms)"

    # P95 target rank = 0.95 * 120 = 114
    # Falls in 250ms bucket (100-105 range)
    assert p95 >= 100.0, f"P95 ({p95}ms) should be at least 100ms"
